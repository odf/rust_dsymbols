...
impl GUI {
    ...
    ///
    pub fn from_gl_context(context: std::sync::Arc<three_d::context::Context>) -> Self {
        GUI {
                    ...
                    Event::MousePress {
                        ...
                        if !handled {
                            Some(egui::Event::PointerButton {
                                ...
                                button: to_egui_mouse_button(button),
                                pressed: true,
                                modifiers: to_egui_modifiers(modifiers),
                            })
                        } else {
    ...
    pub fn render(&self) -> Result<(), three_d::CoreError> {
        ...
        unsafe {
            ...
            self.painter.borrow().gl().disable(egui_glow::glow::FRAMEBUFFER_SRGB);
        }
        Ok(())
    }
}
...
fn to_egui_key(key: &Key) -> egui::Key {
    use three_d::control::Key::*;
    use egui::Key;
...
fn to_egui_mouse_button(button: &MouseButton) -> egui::PointerButton {
    match button {
